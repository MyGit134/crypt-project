...
def formdeckey(passwd: str, data: list):
    ...
    deckey = base64.b85encode(SHA3_512.new(
        b'<KS>' + password + mac + cpu_name + disk_serial + mb_serial + gpu_serial + system_uuid + b'<KE>')
                              .digest()).decode('utf-8')
    return deckey
...
def formenckey(passwd: bytes, data: list):
    ...
    disk_serial = data[3]
    mb_serial = data[4]
    enckey = base64.b85encode(SHA3_512.new(
        b'<KS>' + password + mac + disk_serial + cpu_name + mb_serial + system_uuid + b'<KE>')
                              .digest()).decode('utf-8')
    return enckey
